...
//#     0< x2 + x3 + x4  < 500
//#     0< x1 + x5 + x6  < 500
//#                  x7 = 0 (belong to second stage senario 2)
//# x free variables
...
int str_eval_f(double* x0, double* x1, double* obj, CallBackDataPtr cbd) {
  ...
  MESSAGE("str_prob_info -- row " << row <<" col "<<col);
  assert(row == col);
  if (row == 0) {   // x1^2 + x2^2 +x1x2
    *obj = x0[0] * x0[0] + x0[1] * x0[1] + x0[0] * x0[1];
  }
  else if (row == 1) {   // x3^2 + x4^2 + x3x4
    *obj = x1[0] * x1[0] + x1[1] * x1[1] + x1[0] * x1[1];
  }
  else if (row == 2) {  // x5^2 + x6^2 + x5x6
    *obj = x1[0] * x1[0] + x1[1] * x1[1] + x1[0] * x1[1];
  }
  else
    assert(false);
...
//    assert(eq_g==NULL);
  ...
  else if (row == 1) {   //x2 + x3 + x4
    inq_g[0] = x0[1] + x1[0] + x1[1];
  }
  else if (row == 2) {  //x1 + x5 + x6
    eq_g[0] = x1[2];
...
int main(int argc, char* argv[]) {
  ...
  PipsNlpProblemStructPtr prob = CreatePipsNlpProblemStruct(MPI_COMM_WORLD, 2, init_x0, prob_info, eval_f, eval_g, eval_grad_f, eval_jac_g, eval_h, write_solution, NULL);
